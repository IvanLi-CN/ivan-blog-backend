...
import { DuplicateEntityException, DuplicateFieldInfo } from '../exceptions/duplicate-entity.exception';

export class BaseDbService<Entity extends {}> extends TypeOrmHelper<Entity> {
  async isDuplicateEntity<Dto = { [p: string]: any }>(
    em: Repository<Entity>, dto: Dto, fields?: Array<keyof Dto & string>,
  ): Promise<false | never> {
  ...
  private static getCompareFields<Dto>(dto: Dto, fields: Array<keyof Dto & string>) {
    const compareFields = [];
  ...
  private async checkDuplicateFields<Dto = { [p: string]: any }>(
    qb: SelectQueryBuilder<Entity>, dto: Dto, compareFields: Array<keyof Dto & string>,
  ): Promise<false | never> {
