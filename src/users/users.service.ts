...
export class UsersService {
  ...
  async getLoggedUser({email, password}: UserLoggingDto): Promise<LoggedUserDto> {
  ...
  async verifyNameAvailable(name: string) {
  ...
  async registerUser({email, name, password} : UserRegisterDto){
    const results = await Promise.all([
      this.verifyEmailAvailable(email),
      this.verifyNameAvailable(name)
    ]);
    if (!results[0]) {
  ...
  async updateUser({name, oldPassword, newPassword}: UpdateUserDto, {id}: LoggedUserDto) {
    const user = await this.userRepository.findOne(id, {select: ['name', 'email', "password"]});
    if (!user) {
    ...
    const rt = await this.userRepository.update(id, {
