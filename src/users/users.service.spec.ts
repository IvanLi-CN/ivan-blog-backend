  ...
  describe('Test UsersService', () => {
    ...
    describe('registerUser', () => {
      ...
      it('should throw name exists exception', async () => {
        ...
        await service.registerUser(user).catch(ex => {
          expect(ex.message).toMatchObject(new BadRequestException('用户名已存在！').message);
        })
      });
      it('should throw email exists exception', async () => {
        ...
        await service.registerUser(user).catch(ex => {
          expect(ex.message).toMatchObject(new BadRequestException('电子邮箱已存在！').message);
        })
      });
    });
    describe('getLoggedUser', () => {
      it('should get a user', async () => {
        return await expect(
          service.getLoggedUser({email: 'test1', password: 'pwd1', captcha: '', remember: true})
        ).resolves.toMatchObject({email: 'test1'})
      });
      it('should throw NotAcceptableException', async () => {
        expect.assertions(2);
        await expect(
          service.getLoggedUser({email: 'test1', password: 'pwd2', captcha: '', remember: true})
        ).rejects.toBeInstanceOf(NotAcceptableException);
        await expect(
          service.getLoggedUser({email: 'test4', password: 'pwd4', captcha: '', remember: true})
        ).rejects.toBeInstanceOf(NotAcceptableException);
      })
    });
    describe('updateUser', () => {
       ...
       await expect(
         new Promise(resolve => {
           repository.findOne({email: 'test1'}).then(user => {
             service.updateUser({name: 'newName', oldPassword: 'pwd1'}, user).then(() => {
               resolve(true);
             })
           });
         })
      ...
      it('should throw UnauthorizedException', async () => {
        await expect(
          (async () => {
            ...
            return await service.updateUser({name: 'newName', oldPassword: 'p'}, user);
          })()
